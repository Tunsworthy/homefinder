(function(){
  ...
  HF.renderListingContent = function(node, item, options){ const opts = Object.assign({commentsMode:'none', showDomain:true, compact:false, showLinks:true, includeCommentEditor:false, skipImages:false}, options||{})
    ...
    el.innerHTML = `${imgHtml}<div class="flex items-center justify-between mb-1"><div class="font-medium text-sm">${HF.escapeHtml(item.address||'')}</div>${statusBadge}</div>${price}${stats}${inspectionSection}<div class="commutes-container mt-2"></div>${actionButtons}${votesSection}${commentsSection}`
    return el
  }
