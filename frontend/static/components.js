(function(){
  ...
  HF.renderListingContent = function(node, item, options){ const opts = Object.assign({commentsMode:'none', showDomain:true, compact:false, showLinks:true, includeCommentEditor:false, skipImages:false}, options||{})
    ...
    const dateHtml = item.added_date ? `<div class="text-xs text-gray-600 mb-1">Date found: <span class="font-medium">${HF.escapeHtml(item.added_date)}</span></div>` : ''
    ...
    const addToPlanBtn = !nextInsp ? `<button class="add-to-plan-btn px-3 py-1 bg-blue-600 text-white rounded text-xs hover:bg-blue-700 mt-2" data-listing-id="${item.id}" data-listing-address="${HF.escapeHtml(item.address||'')}" data-listing-suburb="${HF.escapeHtml(item.suburb||'')}">📋 Add to Plan</button>` : ''
    ...
    el.innerHTML = `${imgHtml}<div class="flex items-center justify-between mb-1"><div class="font-medium text-sm">${HF.escapeHtml(item.address||'')}</div>${statusBadge}</div>${price}${dateHtml}${stats}<div class="commutes-container mt-2"></div>${actionButtons}${votesSection}${inspectionSection}${addToPlanBtn}${commentsSection}`
    return el
  }
